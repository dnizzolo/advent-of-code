...
(defclass conjunction (module)
  ((lasts :accessor conjunction-lasts)
   (inputs :accessor conjunction-inputs)
   (sent-high-p :initform nil :accessor conjunction-sent-high-p)))

(defun conjunction-p (object)
  (typep object 'conjunction))

(defmethod initialize-instance :after ((instance conjunction) &key)
  (setf (slot-value instance 'lasts)
        (make-array 4 :adjustable t :fill-pointer 0)
        (slot-value instance 'inputs)
        (make-array 4 :adjustable t :fill-pointer 0)))
...
(defmethod process-pulse (pulse sender (module conjunction))
  (with-slots (lasts inputs) module
    ...
    (let ((pulse (if (every (lambda (pin) (eql pin :high)) lasts) :low :high)))
      (when (eql pulse :high)
        (setf (conjunction-sent-high-p module) t))
...
(defun schedule-output-pulse (pulse sender)
...
(defun push-button (modules)
  (setf *the-agenda* (q:make-queue))
  (schedule-pulse :low :button (find-if #'broadcaster-p modules))
  (simulate))

(defun push-button-1000-times (modules)
  (loop initially (setf *low-pulse-count* 0
                        *high-pulse-count* 0)
        repeat 1000
        do (push-button modules)
        finally (return (prog1 (* *low-pulse-count* *high-pulse-count*)
                          (setf *low-pulse-count* 0
                                *high-pulse-count* 0)))))
