...
(defun make-polymer-counter (template &aux (counter (make-hash-table)))
    (loop for poly in template
          do (incf (gethash poly counter 0))
          finally (return counter)))

(defun apply-polymerization-rules (template rules n)
  (loop with adj = (make-adjacent-polymers-hash-table template)
        with counter = (make-polymer-counter template)
        repeat n
